...
function validateStartDate(weekId = program.currentWeek) {
  ...
  // Prevent overlapping with other weeks (exclude current week)
  const allWeeks = Object.values(program.weeks);
  for (let w of allWeeks) {
    if (!w.startDate) continue;
    if (w.id === weekId) continue; // ❌ This is the fix
...
function completeDay(weekId, day, result) {
  if (!confirm("Have you completed this workout?")) return;
  ...
  weekIds.forEach(id => {
    ...
    weekDiv.innerHTML = `
      <div style="display:flex; justify-content:space-between; align-items:center;">
        ...
        ${!isCurrent ? `<button onclick="toggleWeek(${id})">Toggle</button>` : ''}
      </div>
